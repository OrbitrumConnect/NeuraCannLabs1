                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-4 gap-4 mb-6">
                      {searchTabs.filter(tab => tab.type === 'main').map((mainTab, mainIndex) => (
                        <div key={mainTab.id} className="relative">
                          ...
                          {/* Main Research Node - Expandido para visualização completa */}
                          <div 
                            className={`research-node relative bg-black/90 backdrop-blur-md rounded-lg border p-6 cursor-pointer transition-all ${
                              activeTabId === mainTab.id 
                                ? 'border-neon-cyan shadow-lg shadow-neon-cyan/20 min-h-[700px] w-full max-w-[900px] mx-auto' 
                                : 'border-gray-600/50 hover:border-neon-cyan/60 min-h-[600px] w-full'
                            }`}
                          ...
                          >
                            ...
                            <div className="mb-4">
                              <TextToSpeech 
                                text={`Análise cruzada sobre ${mainTab.query}: ${mainTab.response
                                  .replace(/[🔬📊🏥⚠️💊🧠🎯•→]/g, '')
                                  .replace(/\*\*/g, '')
                                  .replace(/\n/g, ' ')
                                  .substring(0, 300)
                            ...
                            {/* Sugestões de Sub-pesquisa - Funcionais */}
                            <div className="grid grid-cols-2 gap-2 mb-4">
                              {mainTab.suggestions && mainTab.suggestions.slice(0, 4).map((suggestion, idx) => (
                                <button 
                                  key={idx}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  className="px-3 py-2 bg-purple-600/20 text-purple-300 border border-purple-500/30 rounded text-xs hover:bg-purple-600/30 transition-all text-left"
                                >
                                  🔍 {suggestion}
                                </button>
                              ))}
